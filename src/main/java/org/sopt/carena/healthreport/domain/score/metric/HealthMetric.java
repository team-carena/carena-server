 ...
 */
/*
lowerExclusive = false → 하한 “이상” (≥)

lowerExclusive = true → 하한 “초과” (>)

upperExclusive = false → 상한 “이하” (≤)

upperExclusive = true → 상한 “미만” (<
 */
@Getter
...
public enum HealthMetric {
    ...
    BMI(new DefaultScorePolicy(
            ...
            gender -> 24.9,
            gender -> 5.0,
            DeviationType.BIDIRECTIONAL,false,false)
            ,0.08
    ),
    //허리둘레
    WAIST_CIRCUMFERENCE(new DefaultScorePolicy(
            ...
            gender -> 5.0,
            DeviationType.UPPER_ONLY,false,true)
            ,0.12
    ),
    ...
    HEMOGLOBIN(new DefaultScorePolicy(
            ...
            gender -> 1.0,
            DeviationType.LOWER_ONLY,false,false),
            0.1
    ),
    ...
    SERUM_CREATININE(new DefaultScorePolicy(
            ...
            gender -> 1.5,
            gender -> 0.3,
            DeviationType.UPPER_ONLY,false,false)
            ,0.045
    ),
    ...
    public ScoreItem calculateResult(Double value, Gender gender) {
        return scorePolicy.calculate(value, gender, this.importance);
