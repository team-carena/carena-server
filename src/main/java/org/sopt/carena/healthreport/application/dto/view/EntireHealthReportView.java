...
public record EntireHealthReportView(
	...
	private static List<DisplayElement> buildBasicInspection(final HealthReport report){
		return List.of(new DisplayElement("height", report.getHeight().value(), null),
	...
	private static List<DisplayElement> buildBloodPressureInspection(final HealthReport report){
		return List.of(new DisplayElement("systolicBloodPressure", report.getBloodPressure().systolicBp(), report.getBloodPressure().systolicBpStatus().getRiskLevel().getDescription()),
	...
	private static List<DisplayElement> buildDiabetesInspection(final HealthReport report){
		return List.of(new DisplayElement("fastingGlucose", report.getFastingGlucose().value(), report.getFastingGlucose().status().getRiskLevel().getDescription()));
	}

	private static List<DisplayElement> buildLiverInspection(final HealthReport report){
		return List.of(new DisplayElement("ast", report.getAst().value(), report.getAst().status().getRiskLevel().getDescription()),
	...
	private static List<DisplayElement> buildKidneyInspection(final HealthReport report){
		return List.of(new DisplayElement("serumCreatinine", report.getSerumCreatinine().value(), report.getSerumCreatinine().status().getRiskLevel().getDescription()),
	...
	private static List<DisplayElement> buildAnemiaInspection(final HealthReport report){
		return List.of(new DisplayElement("hemoglobin", report.getHemoglobin().value(), report.getHemoglobin().status().getRiskLevel().getDescription()));
	}
